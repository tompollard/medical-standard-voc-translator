...
class Standard_vocab_translator(object):
    ...
    def __init__(self, source_vocabulary: str, target_vocabulary: str, source_file: str, source_file_code_column: str):
        ...
        source_voc = pd.read_csv(source_file, converters={
                                 source_file_code_column: str})
        ...
        concept_id_source = source_voc.merge(
            concept, how='left', left_on=source_file_code_column, right_on="concept_code")

        # 2 step. Merge tables joining on concept_id(from source voc) to concept_id()
        source_rel_merge = concept_id_source.merge(
            concept_rel, how='left', left_on="concept_id", right_on="concept_id_1")

        # Cleaning undesired columns
        source_rel_merge = source_rel_merge[[
            source_file_code_column, 'concept_id', 'concept_id_1', 'concept_id_2']]
        ...
        rel_target_merge = source_rel_merge.merge(
            concept, how='left', left_on="concept_id_2", right_on="concept_id")

        rel_target_merge = rel_target_merge[[
            source_file_code_column, 'concept_id_x', 'concept_id_1', 'concept_id_2', 'concept_code']]
